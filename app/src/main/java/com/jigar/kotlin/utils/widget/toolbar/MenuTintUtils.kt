...
object MenuTintUtils {
        ...
        for (i in 0 until menu.size()) {
            ...
            if (item.subMenu != null && item.subMenu.size() > 0) {
                ...
                for (j in 0 until subMenu.size()) {
                    ...
                    applyFontToMenuItem(subMenuItem, tintColorAlpha, context)
                }
            } else {
                tintMenuItemIcon(color, item)
                applyFontToMenuItem(item, color, context)
            }
    ...
    private fun applyFontToMenuItem(mi: MenuItem, color: Int, context: Context) {
        ...
        val typeface: Typeface = ResourcesCompat.getFont(context, R.font.font_bold)!!
        ...
        val textSize: Int = context.resources.getDimensionPixelSize(R.dimen.textSizeMedium)
